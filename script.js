...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const skillList = document.getElementById('cms-skill-list');
        if(skillList && data.about.skills) {
            skillList.innerHTML = '';
            data.about.skills.forEach(skill => {
                skillList.innerHTML += `
                <li>
                    <span class="skill-name">${skill.name[lang]}</span>
        ...
        if (portfolioGrid && data.portfolio.items) {
            ...
            data.portfolio.items.forEach(port => {
                portfolioGrid.innerHTML += `
                <div class="portfolio-item ${port.category}">
                    <img src="${port.image}" alt="${port.title[lang]}" loading="lazy">
                    <div class="portfolio-overlay">
                        <div class="portfolio-info">
                            <h3>${port.title[lang]}</h3>
                            <p>${port.subtitle[lang]}</p>
